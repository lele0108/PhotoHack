...
if(!empty($_POST)  && empty($error)) {
	...
	if(empty($post_data['error'])) {
		...
		try{
			$result = $FlashFotoAPI->addByFile($_FILES['image']['tmp_name'], $_FILES['image']['type'], $post_data['api_params'] ? $post_data['api_params'] : null);
		} catch(Exception $e) {
...
<html>
	...
	<body>
		...
		<h2>
			...
			By File (Multipart form) Example - FlashFoto PHP API SDK
		</h2>
